-- Abandoned cart emails: $150 off, code 150off, professional copy (no emojis).
-- Run in Supabase SQL Editor. Does not change assessment or no_convert templates.
...
SET
  ...
  subject = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
    REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
      ...
      '200OFF', '150off'), '100OFF', '150off'), '75OFF', '150off'), '150OFF', '150off'),
...
-- Promo code for checkout (case-insensitive validation supports 150off / 150OFF)
DELETE FROM promo_codes WHERE UPPER(TRIM(code)) IN ('150OFF', '150off');
...
-- Verify
SELECT template_type, course_slug, discount_code, discount_amount, LEFT(subject, 70) AS subject_preview
