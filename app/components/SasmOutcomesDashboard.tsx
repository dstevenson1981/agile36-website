...
const SALARY_LADDER = [
  { role: "Scrum Master", band: 115, label: "$115K" },
  { role: "SAFe SSM", band: 130, label: "$130K" },
  { role: "SASM", band: 145, label: "$145K", highlight: true },
  { role: "Agile Coach", band: 160, label: "$160K" },
  { role: "RTE path", band: 180, label: "$180K+" },
];
    ...
    <div
      className={
        className.includes("bg-")
          ...
          : `liquid-glass rounded-xl ${className}`
      }
    >
      {children}
    </div>
...
function KpiTile({
  ...
  return (
    <DashboardCard className="p-5">
      ...
      <p className="mt-2 text-[1.5rem] font-semibold tracking-tight text-[#1f2c4a] tabular-nums">
        {prefix}
        {value}
        {suffix}
      </p>
      <p className="mt-1.5 text-[13px] leading-snug text-slate-600">{hint}</p>
    </DashboardCard>
  );
}
    ...
    <DashboardCard className="p-5 sm:p-6">
      <div className="mb-1 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 text-base font-semibold tracking-tight text-[#1f2c4a]">
...
function OutcomesCta() {
  return (
    <DashboardCard className="flex flex-col justify-between bg-[#1f2c4a] p-6 text-white lg:col-span-1">
      <div>
...
/**
 * Outcomes dashboard for the SASM course page.
...
export default function SasmOutcomesDashboard() {
  return (
    <section className="relative w-full overflow-hidden border-y border-[#1f2c4a]/10 bg-black px-4 py-16 sm:px-6 lg:px-20">
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute -left-24 top-0 h-72 w-72 rounded-full bg-[#d97706]/[0.08] blur-[100px]" />
        <div className="absolute -right-16 bottom-0 h-80 w-80 rounded-full bg-sky-400/[0.07] blur-[110px]" />
      </div>
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <KpiTile label="Pass threshold" value={73} suffix="%" hint="Official SASM exam passing score" />
          <KpiTile label="Live training" value={16} suffix=" hrs" hint="2 days with a SAFe Program Consultant" />
          <KpiTile label="Salary midpoint" value={145} prefix="$" suffix="K" hint="Typical US band for SASM-level roles" />
          <KpiTile label="Exam window" value={30} suffix=" days" hint="Complete your attempt after class ends" />
        </div>
