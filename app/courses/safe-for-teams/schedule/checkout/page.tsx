...
function CheckoutContent() {
  ...
  const courseSlug = searchParams.get('course') || 'safe-for-teams';
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/safe-for-teams/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/safe-for-teams/schedule/checkout/success?payment_intent=${paymentIntentId}`,
                ...
                >
                  <PaymentForm
                    onSuccess={() => {
                      ...
                      router.push(`/courses/safe-for-teams/schedule/checkout/success?${params.toString()}`);
                    }}
                    onCancel={() => {
                      router.push(`/courses/safe-for-teams/schedule?course=${courseSlug}`);
                    }}
