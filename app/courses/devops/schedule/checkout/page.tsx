...
function CheckoutContent() {
  ...
  const courseSlug = searchParams.get('course') || 'devops';
  ...
  const [courseName, setCourseName] = useState("SAFe DevOps");
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/devops/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/devops/schedule/checkout/success?payment_intent=${paymentIntentId}`,
                ...
                >
                  <PaymentForm
                    onSuccess={() => {
                      ...
                      router.push(`/courses/devops/schedule/checkout/success?${params.toString()}`);
                    }}
                    onCancel={() => {
                      router.push(`/courses/devops/schedule?course=${courseSlug}`);
                    }}
