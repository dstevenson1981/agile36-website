...
function CourseScheduleContent() {
  ...
  const courseSlug = searchParams.get('course') || 'advanced-scrum-master';
  const [schedules, setSchedules] = useState<any[]>([]);
  const [filteredSchedules, setFilteredSchedules] = useState<any[]>([]);
  const [isLoadingSchedules, setIsLoadingSchedules] = useState(true);
  const [displayedCount, setDisplayedCount] = useState(10); // Show 10 initially
  const [courseName, setCourseName] = useState("Advanced Scrum Master Certification Path");
      ...
      <section className="w-full bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 py-6">
          ...
          <div className="flex items-center gap-2 mb-4 text-sm text-gray-600">
            ...
            <span>/</span>
            <Link href="/courses/advanced-scrum-master" className="hover:text-[#01203d]">Advanced Scrum Master Certification Path</Link>
            <span>/</span>
                      ...
                      <div key={schedule.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-shadow">
                        <div className="flex flex-col lg:flex-row gap-6">
                          ...
                          <div className="w-full lg:w-80 flex-shrink-0 border-t lg:border-t-0 lg:border-l border-gray-200 pt-6 lg:pt-0 lg:pl-6 space-y-4">
                            ...
                            <Link
                              href={`/courses/advanced-scrum-master/schedule/checkout?schedule=${schedule.id}&course=advanced-scrum-master&quantity=${qty}`}
