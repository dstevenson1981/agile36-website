...
function CheckoutContent() {
  ...
  const courseSlug = searchParams.get('course') || 'scrum-master';
  ...
  const [courseName, setCourseName] = useState("AI-Empowered SAFe Scrum Master");
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/scrum-master/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/scrum-master/schedule/checkout/success?payment_intent=${paymentIntentId}`,
              ...
              <div className="rounded-2xl border border-[#1f2c4a]/15 bg-[#1f2c4a]/[0.06] p-6">
                ...
                >
                  <PaymentForm
                    ...
                    onCancel={() => {
                      router.push(`/courses/scrum-master/schedule?course=${courseSlug}`);
                    }}
