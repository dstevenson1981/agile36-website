  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/ai-agent-builder/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/ai-agent-builder/schedule/checkout/success?payment_intent=${paymentIntentId}`,
                ...
                >
                  <PaymentForm
                    onSuccess={() => {
                      ...
                      router.push(`/courses/ai-agent-builder/schedule/checkout/success?${params.toString()}`);
                    }}
                    onCancel={() => {
                      router.push(`/courses/ai-agent-builder/schedule?course=${courseSlug}`);
                    }}
