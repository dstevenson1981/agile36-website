...
function CheckoutContent() {
  ...
  const courseSlug = searchParams.get('course') || 'product-owner-manager';
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/product-owner-manager/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/product-owner-manager/schedule/checkout/success?payment_intent=${paymentIntentId}`,
              ...
              <div className="rounded-2xl border border-[#1f2c4a]/15 bg-[#1f2c4a]/[0.06] p-6">
                ...
                >
                  <PaymentForm
                    ...
                    onCancel={() => {
                      router.push(`/courses/product-owner-manager/schedule?course=${courseSlug}`);
                    }}
