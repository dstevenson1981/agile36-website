  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      if (!scheduleId) {
        ...
        router.push(`/courses/safe-for-architects/schedule`);
        return;
      }
  ...
  const createPaymentIntent = async () => {
    ...
    try {
      ...
      const handled = await handleCreatePaymentIntentResult(data, {
        ...
        successUrl: (paymentIntentId) => `/courses/safe-for-architects/schedule/checkout/success?payment_intent=${paymentIntentId}`,
                ...
                >
                  <PaymentForm
                    onSuccess={() => {
                      ...
                      router.push(`/courses/safe-for-architects/schedule/checkout/success?${params.toString()}`);
                    }}
                    onCancel={() => {
                      router.push(`/courses/safe-for-architects/schedule?course=${courseSlug}`);
                    }}
