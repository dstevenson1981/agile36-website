...
function quantitiesFrom(rows: CourseScheduleRow[]): { [key: string]: number } {
  const initial: { [key: string]: number } = {};
  rows.forEach((schedule) => {
    initial[schedule.id] = 1;
  });
  return initial;
}
...
function CourseScheduleContent({
  ...
  const [filteredSchedules, setFilteredSchedules] = useState<CourseScheduleRow[]>(initialSchedules);
  const [isLoadingSchedules, setIsLoadingSchedules] = useState(initialSchedules.length === 0);
  ...
  const [selectedScheduleForInquiry, setSelectedScheduleForInquiry] = useState<CourseScheduleRow | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      if (initialSchedules.length === 0) setIsLoadingSchedules(true);
      try {
        let response: Response | null = null;
        for (let attempt = 0; attempt < 2; attempt++) {
          response = await fetch(
            `/api/course-schedules?course_slug=${courseSlug}&status=active&include_hidden=1&_t=${Date.now()}`,
            { cache: "no-store" }
          );
    ...
    const displayName = COURSE_NAMES[courseSlug] || 'SAFe AI-Empowered Release Train Engineer';
    setCourseName(displayName);
    fetchSchedules();
    return () => {
      cancelled = true;
    };
  }, [courseSlug, initialSchedules.length]);
  ...
  const handleGroupInquiryClick = (schedule: CourseScheduleRow) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 py-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <div className="ml-auto flex items-center gap-2 text-sm text-[#64748b]">
              <span>20% off for any group of 5 or more</span>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1">
              {isLoadingSchedules ? (
                ...
                <div className="text-center py-12 liquid-glass rounded-2xl px-6">
                  <svg className="mx-auto h-12 w-12 text-[#64748b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                  </svg>
                  <h3 className="mt-2 text-lg font-medium text-[#1f2c4a]">Private class</h3>
                  <p className="mt-2 text-sm text-[#64748b] max-w-md mx-auto">
                    RTE cohorts are arranged privately. Contact us to discuss dates, pricing, and registration — schedules are not listed for open enrollment.
                  </p>
                  <Link
                    href="/contact?course=release-train-engineer"
                    className="mt-5 inline-flex items-center justify-center rounded-lg bg-[#1f2c4a] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#16243f]"
                  >
...
export default function PrivateRteScheduleClient({
  initialSchedules,
}: {
  initialSchedules: CourseScheduleRow[];
}) {
  return (
    <Suspense fallback={
      ...
      <CourseScheduleContent initialSchedules={initialSchedules} />
    </Suspense>
  );
}
