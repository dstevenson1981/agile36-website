...
export default function AdvancedScrumMasterPracticeTest({
  ...
  const question = ADVANCED_SCRUM_MASTER_QUESTIONS[currentIndex];
  const total = ADVANCED_SCRUM_MASTER_QUESTIONS.length;
  ...
  if (showResults) {
    const correct = ADVANCED_SCRUM_MASTER_QUESTIONS.filter((q) => answers[q.id] === q.correctIndex).length;
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {score >= passingScore
                  ? "Great job! You're on track for the SASM certification exam."
                  : 'Review the questions below and try again when ready.'}
              </p>
            </div>
          </div>
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            {ADVANCED_SCRUM_MASTER_QUESTIONS.map((q) => {
