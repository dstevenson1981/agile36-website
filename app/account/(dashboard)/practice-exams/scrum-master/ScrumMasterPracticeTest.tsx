...
export default function ScrumMasterPracticeTest() {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const [showResults, setShowResults] = useState(false);

  const question = SCRUM_MASTER_QUESTIONS[currentIndex];
  const total = SCRUM_MASTER_QUESTIONS.length;
  ...
  if (showResults) {
    const correct = SCRUM_MASTER_QUESTIONS.filter((q) => answers[q.id] === q.correctIndex).length;
    const score = Math.round((correct / total) * 100);
    const passingScore = 73;
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {score >= passingScore
                  ? "Great job! You're on track for the SAFe Scrum Master certification exam."
                  : 'Review the questions below and try again when ready.'}
              </p>
            </div>
          </div>
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            {SCRUM_MASTER_QUESTIONS.map((q) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm">
          ...
          {question.options.map((option, idx) => {
            ...
            return (
              <li key={idx}>
                ...
                >
                  <span className="font-medium text-slate-500 flex-shrink-0 w-6">{OPTION_LETTERS[idx]}.</span>
                  <span className="flex-1">{option}</span>
                  {hasAnswered && isCorrect && (
                    <span className="text-green-600 font-bold flex-shrink-0" title="Correct answer">
                      ✔
                    </span>
                  )}
                </button>
        ...
        {answers[question.id] !== undefined && (
          <p
            className={`mt-4 text-sm font-medium ${answers[question.id] === question.correctIndex ? 'text-green-600' : 'text-amber-600'}`}
          >
            {answers[question.id] === question.correctIndex
              ? '✓ Correct!'
              : `The correct answer is ${OPTION_LETTERS[question.correctIndex]}.`}
          </p>
        )}
      </div>
