...
type SafeForTeamsPracticeTestProps = {
  backHref?: string;
  backLabel?: string;
};

export default function SafeForTeamsPracticeTest({
  backHref = '/account/practice-exams',
  backLabel = 'Back to Practice Exams',
}: SafeForTeamsPracticeTestProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const [showResults, setShowResults] = useState(false);

  const question = SAFE_FOR_TEAMS_QUESTIONS[currentIndex];
  const total = SAFE_FOR_TEAMS_QUESTIONS.length;
  ...
  if (showResults) {
    const correct = SAFE_FOR_TEAMS_QUESTIONS.filter((q) => answers[q.id] === q.correctIndex).length;
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {score >= passingScore
                  ? "Great job! You're on track for the SAFe for Teams (SP) certification exam."
                  : 'Review the questions below and try again when ready.'}
              </p>
            </div>
          </div>
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
            {SAFE_FOR_TEAMS_QUESTIONS.map((q) => {
        ...
        {answers[question.id] !== undefined && (
          <p
            className={`mt-4 text-sm font-medium ${
              answers[question.id] === question.correctIndex ? 'text-green-600' : 'text-amber-600'
            }`}
          >
