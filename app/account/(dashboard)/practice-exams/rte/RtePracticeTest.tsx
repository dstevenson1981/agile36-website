...
type RtePracticeTestProps = {
  backHref?: string;
  backLabel?: string;
};

export default function RtePracticeTest({
  backHref = '/account/practice-exams',
  backLabel = 'Back to Practice Exams',
}: RtePracticeTestProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<number, number>>({});
  const [showResults, setShowResults] = useState(false);

  const question = RTE_QUESTIONS[currentIndex];
  const total = RTE_QUESTIONS.length;
  ...
  if (showResults) {
    const correct = RTE_QUESTIONS.filter((q) => answers[q.id] === q.correctIndex).length;
